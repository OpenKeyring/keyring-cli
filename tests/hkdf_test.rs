...
fn device_id_boundary_long_device_id() {
    ...
    assert_eq!(
        key.len(),
        32,
        "Long device ID should produce 32-byte key"
    );
}

#[test]
fn integration_derived_key_can_encrypt_decrypt() {
    use keyring_cli::crypto::{encrypt, decrypt};
    ...
    let (ciphertext, nonce) = encrypt(plaintext, &device_key)
        .expect("Derived key should be able to encrypt");

    // Use derived key to decrypt data
    let decrypted = decrypt(&ciphertext, &nonce, &device_key)
        .expect("Derived key should be able to decrypt");
...
fn integration_different_device_keys_produce_different_ciphertexts() {
    ...
    let (ciphertext1, _nonce1) = encrypt(plaintext, &device_key_1)
        .expect("Should encrypt with device key 1");
    let (ciphertext2, _nonce2) = encrypt(plaintext, &device_key_2)
        .expect("Should encrypt with device key 2");
...
fn hkdf_produces_cryptographically_strong_keys() {
    ...
    assert_ne!(hash1, hash2, "Similar device IDs should produce very different keys");
    assert_ne!(hash2, hash3, "Similar device IDs should produce very different keys");
    assert_ne!(hash1, hash3, "Similar device IDs should produce very different keys");
