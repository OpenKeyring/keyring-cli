...
///
/// # Platform Notes
/// macOS has stricter limits on mlock than Linux.
...
pub fn protect_memory(addr: *mut u8, len: usize) -> Result<()> {
    ...
    if result != 0 {
        ...
        return Err(PlatformError::MemoryProtectionFailed(format!(
            "mlock failed with errno {}: {}. Hint: macOS has strict limits on mlock. \
            Consider reducing allocation size or using encrypted keychain APIs.",
            errno,
...
/// Get the maximum amount of memory that can be locked
///
/// macOS has limits on the total amount of memory that can be locked.
/// This function queries the system limits.
///
/// # Returns
/// The maximum number of bytes that can be locked, or 0 if unable to determine.
pub fn max_locked_memory() -> usize {
    unsafe {
...
mod tests {
    ...
    fn test_page_size() {
        ...
        // macOS typically uses 4KB or 16KB pages
        assert!(page == 4096 || page == 16384, "Unexpected page size");
    }

    #[test]
    fn test_max_locked_memory() {
        let max = max_locked_memory();
        // macOS should return a non-zero limit
        // The exact value varies by system configuration
        if max != 0 {
